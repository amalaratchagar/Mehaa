...
public class IndexModel : PageModel
{
    ...
    public async Task<PartialViewResult> OnGetViewAllPartial()
    {
        ...
        {
            ViewName = "_ViewAll",
            ViewData = new ViewDataDictionary<IEnumerable<Core.Entities.Customer>>(ViewData, Customers)
        };
    }
    public async Task<JsonResult> OnGetCreateOrEditAsync(int id = 0)
    {
        if (id == 0)
            return new JsonResult(new { isValid = true, html = await _renderService.ToStringAsync("_CreateOrEdit", new Core.Entities.Customer()) });
        else
        {
    ...
    public async Task<JsonResult> OnPostCreateOrEditAsync(int id, Core.Entities.Customer customer)
    {
